 ...
 * Plugin class
 */
namespace Phile\Plugin\Jacmoe\RssFeed;
...
class Plugin extends \Phile\Plugin\AbstractPlugin implements \Phile\Gateway\EventObserverInterface {
	...
	private function render_file($filename, $vars = null) {
		...
		ob_start();
		include \Phile\Utility::resolveFilePath("MOD:jacmoe/rssFeed/".$filename);
		return ob_get_clean();
	}
	...
	public function on($eventKey, $data = null) {
		...
		if ($eventKey == 'plugins_loaded') {
			...
			if ($uri == $this->settings['feed_url']) {
				$pageRespository = new \Phile\Repository\Page($this->settings);
				...
				//usort($this->config['pages'], $this->settings);
